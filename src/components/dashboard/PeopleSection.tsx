...
function avatarColor(name: string): string {
  const colors = ['bg-brand-100 text-brand-700', 'bg-blue-100 text-blue-700', 'bg-violet-100 text-violet-700', 'bg-amber-100 text-amber-700', 'bg-rose-100 text-rose-700', 'bg-cyan-100 text-cyan-700'];
  let hash = 0;
    ...
    <div className="h-full flex">
        ...
        <div className="p-4 border-b border-surface-100">
          <div className="relative">
            ...
            <input type="text" placeholder="Search contacts..."
              ...
              className="w-full pl-10 pr-4 py-2 border border-surface-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-500" />
          </div>
          ...
          filtered.map(contact => (
            <button key={contact.id} onClick={() => setSelected(contact)}
              className={'w-full text-left px-4 py-3 border-b border-surface-50 hover:bg-surface-50 transition-colors '
                + (selected?.id === contact.id ? 'bg-brand-50' : '')}>
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-surface-900 text-sm truncate">{contact.name || formatPhone(contact.phone)}</p>
                  <p className="text-xs text-surface-400">{formatPhone(contact.phone)}</p>
                </div>
                {contact.message_count > 0 && (
                  <span className="text-xs text-surface-400">{contact.message_count} msgs</span>
                )}
              </div>
            </button>
          ))
      ...
      {selected ? (
        <div className="flex-1 p-6 overflow-y-auto">
          <button onClick={() => setSelected(null)} className="md:hidden text-sm text-brand-600 mb-4">Back</button>
          <div className="max-w-lg">
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <h2 className="text-lg font-semibold text-surface-900">{selected.name || 'Unknown'}</h2>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="bg-surface-50 rounded-lg p-3">
                ...
                <p className="text-lg font-semibold text-surface-900">{selected.message_count || 0}</p>
              </div>
              <div className="bg-surface-50 rounded-lg p-3">
            ...
            {selected.notes && (
              <div className="mb-4">
                ...
                <p className="text-sm text-surface-600">{selected.notes}</p>
              </div>
            )}
          </div>
        </div>
